...
game = False
TOKEN = "7220953758:AAH8jXqX5_YNIzuUbx_o-XIlTxyPJ9bk73I"
bot = TeleBot(TOKEN)
night = False
...
def game_loop(message):
    ...
    while True:
        ...
        if not night:
            bot.send_message(message.chat.id, "Город засыпает, просывается мафия. Наступила ночь")
        else:
...
def kill(message):
    username = ''.join(message.test.split(' ')[1:])
